...
class SimpleRealm {
    ...
    function stretch($url) {
        $this->user = $this->getCommonPath($this->user, $url->getPath());
    }
    ...
    function isWithin($url) {
        if ($this->isIn($this->user, $url->getBasePath())) {
            return true;
        }
        if ($this->isIn($this->user, $url->getBasePath() . $url->getPage() . '/')) {
            return true;
        }
        return false;
    }
...
class SimpleAuthenticator {
     ...
     *    base URL as the user of a file tree shaped realm.
