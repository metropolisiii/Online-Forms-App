<?php 
	/**
	* Header for all files that include it. Takes care of a lot of event handling, importation of functions, importing of global variables, and database conntections.
	*
		...
		<script>
			$(document).ready(function(){
				...
				$('#form-builder').formbuilder({
					'save_url': 'save.php',
					'load_url': 'load2.php<?php if ($_GET['id']) echo "?id=".$_GET['id']; ?>'
				});
				...
				$('#rightclick').mousedown(function(event){
					switch (event.which) {
						case 3:
							...
							$.post('scripts/write_report.php', {data:serialstring, formid:'<?php echo $_GET['formid']; ?>'},function(){});
							break;
					}
				});
				...
				$('#login_form').submit(function(){
					$('.required').each(function(){
						if ($(this).val()==''){
							alert("Please enter an account for which you want to administer forms.");
							return false;
						}
					});
				});
	...
	<body>
		...
		<div id="form_tabs2">
			<ul class='tabs'>
				...
				<li class='<?php if ($selected=='reports') echo "selected ";?> last'><a href='reports.php'>Reports</a></li>
		</ul>
		</div>
		<?php endif; ?>
		<div id='form_container'>
		<h1>Form Administration</h1>
		<?php if (($user_login_required && isset($_SESSION['userid'])) || $_SESSION['membertype']=='admin'): ?>
		<div id='logout'>
