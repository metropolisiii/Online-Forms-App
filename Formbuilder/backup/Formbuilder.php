	...
	public function generate_html($form_action = false, $form_id=false){
		...
		if(is_array($this->_form_array['form_structure'])){
			...
			if (!$payment)
				...
				$html .= '<?php if ($_SESSION["membertype"] == "admin"): ?><div class="btn-submit"><input type="submit" id="submitform2" name="submit" value="Submit" /></div><?php else: ?><div class="btn-submit"><input type="submit" id="submitform2" name="'.$payment_processor.'" value="Submit and Continue to Payment" /></div><?php endif; ?>'."\n";
			}
			$html .=  '</div>'."\n" ;
