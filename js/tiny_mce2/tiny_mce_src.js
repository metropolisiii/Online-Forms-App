...
(function(win) {
	...
	var tinymce = {
		...
		create : function(s, p, user) {
			...
			ns = t.createNS(s[3].replace(/\.\w+$/, ''), user);
...
tinymce.util.Quirks = function(editor) {
	...
	function cleanupStylesWhenDeleting() {
		function removeMergedFormatSpans(isDelete) {
			...
			// On delete clone the user span of the next block element
			if (isDelete)
				blockElm = dom.getNext(blockElm, dom.isBlock);

			// Locate user span element and clone it since it would otherwise get merged by the "apple-style-span" on delete/backspace
			if (blockElm) {
	...
	function removeBlockQuoteOnBackSpace() {
		...
		editor.onKeyDown.add(function(editor, e) {
			var rng, container, offset, user, parent;
			...
			while (parent && parent.parentNode && parent.parentNode.firstChild == parent && parent.parentNode != user) {
	...
	function addBrAfterLastLinks() {
		function fixLinks(editor, o) {
			tinymce.each(dom.select('a'), function(node) {
				var parentNode = node.parentNode, user = dom.getuser();

				if (parentNode.lastChild === node) {
					while (parentNode && !dom.isBlock(parentNode)) {
						if (parentNode.parentNode.lastChild !== parentNode || parentNode === user) {
							return;
						}
	...
	function setDefaultBlockType() {
		if (settings.forced_user_block) {
			editor.onInit.add(function() {
				setEditorCommandState('DefaultParagraphSeparator', settings.forced_user_block);
			});
		}
	}
	...
	function fakeImageResize() {
		var selectedElmX, selectedElmY, selectedElm, selectedElmGhost, selectedHandle, startX, startY, startW, startH, ratio,
			resizeHandles, width, height, userDocument = document, editableDoc = editor.getDoc();
		...
		function endResize() {
			...
			if (userDocument != editableDoc) {
				dom.unbind(userDocument, 'mousemove', resizeElement);
				dom.unbind(userDocument, 'mouseup', endResize);
			}
			...
			tinymce.each(resizeHandles, function(handle, name) {
				...
				if (!handleElm) {
					...
					dom.bind(handleElm, 'mousedown', function(e) {
						...
						if (userDocument != editableDoc) {
							dom.bind(userDocument, 'mousemove', resizeElement);
							dom.bind(userDocument, 'mouseup', endResize);
						}
					});
				} else {
					dom.show(handleElm);
...
(function(tinymce) {
	...
	function walk(node, user_node, prev) {
		...
		if (node !== user_node) {
			...
			for (parent = node.parent; parent && parent !== user_node; parent = parent.parent) {
...
(function(tinymce) {
	...
	tinymce.html.DomParser = function(settings, schema) {
		...
		settings.user_name = settings.user_name || 'body';
		self.schema = schema = schema || new tinymce.html.Schema();
		...
		self.parse = function(html, args) {
			var parser, userNode, node, nodes, i, l, fi, fl, list, name, validate,
				blockElements, startWhiteSpaceRegExp, invalidChildren = [], isInWhiteSpacePreservedElement,
				endWhiteSpaceRegExp, allWhiteSpaceRegExp, isAllWhiteSpaceRegExp, whiteSpaceElements, children, nonEmptyElements, userBlockName;
			...
			validate = settings.validate;
			userBlockName = "forced_user_block" in args ? args.forced_user_block : settings.forced_user_block;
			...
			function adduserBlocks() {
				var node = userNode.firstChild, next, userBlockNode;
				...
				while (node) {
					...
					if (node.type == 3 || (node.type == 1 && node.name !== 'p' && !blockElements[node.name] && !node.attr('data-mce-type'))) {
						if (!userBlockNode) {
							// Create a new user block element
							userBlockNode = createNode(userBlockName, 1);
							userNode.insert(userBlockNode, node);
							userBlockNode.append(node);
						} else
							userBlockNode.append(node);
					} else {
						userBlockNode = null;
			...
			parser = new tinymce.html.SaxParser({
				...
				start: function(name, attrs, empty) {
					var newNode, attformltersLen, elementRule, textNode, attrName, text, sibling, parent;
					...
					if (elementRule) {
						...
						attformltersLen = attributeFilters.length;
						while (attformltersLen--) {
							attrName = attributeFilters[attformltersLen].name;
			...
			userNode = node = new Node(args.context || settings.user_name, 11);
			...
			// Wrap nodes in the user into block elements if the user is body
			if (userBlockName && userNode.name == 'body')
				adduserBlocks();
...
tinymce.dom.TreeWalker = function(start_node, user_node) {
	var node = start_node;
	...
	function findSibling(node, start_name, sibling_name, shallow) {
		...
		if (node) {
			...
			if (node != user_node) {
				...
				// Walk up the parents to look for siblings
				for (parent = node.parentNode; parent && parent != user_node; parent = parent.parentNode) {
...
*/
		...
		getuser : function() {
			...
			return (s && t.get(s.user_element)) || t.doc.body;
		},
		...
		select : function(pa, s) {
			...
			return tinymce.dom.Sizzle(pa, t.get(s) || t.get(t.settings.user_element) || t.doc, []);
		},
		...
		destroy : function(s) {
			...
			t.win = t.doc = t.user = t.events = t.frag = null;
...
(function(ns) {
	...
	function Range(dom) {
		...
		function _compareBoundaryPoints(containerA, offsetA, containerB, offsetB) {
			var c, offsetC, n, cmnuser, childA, childB;
			...
			cmnuser = dom.findCommonAncestor(containerA, containerB);
			childA = containerA;

			while (childA && childA.parentNode != cmnuser)
			...
			childB = containerB;
			while (childB && childB.parentNode != cmnuser)
		...
		function _setEndPoint(st, n, o) {
			...
			// If one boundary-point of a Range is set to have a user container
			// other than the current one for the Range, the Range is collapsed to
			// the new position. This enforces the restriction that both boundary-
			// points of a Range must have the same user container.
			ec = t[END_CONTAINER];
			while (ec.parentNode)
				ec = ec.parentNode;
		...
		function _traverseRightBoundary(user, how) {
		...
		function _traverseLeftBoundary(user, how) {
	...
	function Selection(selection) {
		...
		this.getBookmark = function(type) {
			...
			function getIndexes(node) {
				var parent, user, children, i, indexes = [];

				parent = node.parentNode;
				user = dom.getuser().parentNode;

				while (parent != user && parent.nodeType !== 9) {
					children = parent.children;
...
var Sizzle = function( selector, context, results, seed ) {
	...
	if ( parts.length > 1 && origPOS.exec( selector ) ) {
		...
		// Take a shortcut and set the context if the user selector is an ID
...
(function(){
	...
	var form = document.createElement("div"),
		...
		user = document.documentElement;

	form.innerHTML = "<a name='" + id + "'/>";

	// Inject it into the user element, check its status, and remove it quickly
	user.insertBefore( form, user.firstChild );
	...
	(function(){
		...
		Sizzle = function( query, context, extra, seed ) {
			...
			if ( !seed && !Sizzle.isXML(context) ) {
				...
				// qSA works strangely on Element-usered queries
				// We can work around this by specifying an extra ID on the user
...
var posProcess = function( selector, context, seed ) {
	var match,
		tmpSet = [],
		later = "",
		user = context.nodeType ? [context] : context;
	...
	for ( var i = 0, l = user.length; i < l; i++ ) {
		Sizzle( selector, user[i], tmpSet, seed );
	}
			...
			update : function(k) {
				...
				if (tinymce.isIE6 && settings.blocker) {
					...
					if (!t.blocker) {
						...
						b = dom.add(settings.container || dom.getuser(), 'iframe', {id : t.blocker, style : 'position:absolute;', frameBorder : 0, src : 'javascript:""'});
	...
	tinymce.create('tinymce.dom.Selection', {
		...
		getBookmark : function(type, normalized) {
			...
			function getLocation() {
				var rng = t.getRng(true), user = dom.getuser(), bookmark = {};
				...
				function getPoint(rng, start) {
					...
					for (; container && container != user; container = container.parentNode)
		...
		moveToBookmark : function(bookmark) {
			var t = this, dom = t.dom, marker1, marker2, rng, user, startContainer, endContainer, startOffset, endOffset;
			...
			function setEndPoint(start) {
				...
				if (point) {
					...
					// Find container node
					for (node = user, i = point.length - 1; i >= 1; i--) {
						children = node.childNodes;
		...
		getSelectedBlocks : function(st, en) {
			...
			if (sb && eb && sb != eb) {
				...
				var walker = new TreeWalker(sb, dom.getuser());
		...
		normalize : function() {
			...
			function normalizeEndPoint(start) {
				var container, offset, walker, dom = self.dom, body = dom.getuser(), node, nonEmptyElementsMap, nodeName;
		...
		selectorChanged: function(selector, callback) {
			...
			if (!self.selectorChangedData) {
				...
				self.editor.onNodeChange.addToTop(function(ed, cm, node) {
					var dom = self.dom, parents = dom.getParents(node, null, dom.getuser()), matchedSelectors = {};
...
(function(tinymce) {
	tinymce.dom.RangeUtils = function(dom) {
		...
		this.walk = function(rng, callback) {
			...
			function findEndPoint(node, user) {
				do {
					if (node.parentNode == user)
						return node;
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.KeyboardNavigation', {
		KeyboardNavigation: function(settings, dom) {
			var t = this, user = settings.user, items = settings.items,
					enableUpDown = settings.enableUpDown, enableLeftRight = settings.enableLeftRight || !settings.enableUpDown,
					excludeFromTabOrder = settings.excludeFromTabOrder,
					itemFocussed, itemBlurred, userKeydown, userFocussed, focussedId;
			...
			t.destroy = function() {
				...
				var userElm = dom.get(user);
				dom.unbind(userElm, 'focus', userFocussed);
				dom.unbind(userElm, 'keydown', userKeydown);

				items = dom = user = t.focus = itemFocussed = itemBlurred = userKeydown = userFocussed = null;
				t.destroy = function() {};
			};
			...
			// Setup initial state for user element.
			if (items[0]){
				focussedId = items[0].id;
			}

			dom.setAttrib(user, 'tabindex', '-1');

			// Setup listeners for user element.
			var userElm = dom.get(user);
			dom.bind(userElm, 'focus', userFocussed);
			dom.bind(userElm, 'keydown', userKeydown);
		}
	});
})(tinymce);
	...
	tinymce.create('tinymce.Editor', {
		...
		initContentBody : function() {
			...
			self.dom = new tinymce.dom.DOMUtils(doc, {
				...
				update_styles : true,
				user_element : settings.content_editable ? self.id : null,
				schema : self.schema
			});
		...
		setContent : function(content, args) {
			var self = this, userNode, body = self.getBody(), forceduserBlockName;
			...
			if (!tinymce.isIE && (content.length === 0 || /^\s+$/.test(content))) {
				forceduserBlockName = self.settings.forced_user_block;
				if (forceduserBlockName)
					content = '<' + forceduserBlockName + '><br data-mce-bogus="1"></' + forceduserBlockName + '>';
				else
...
(function(tinymce) {
	...
	tinymce.Editor.prototype.bindNativeEvents = function() {
		...
		each(nativeToDispatcherMap, function(dispatcherName, nativeName) {
			var user = settings.content_editable ? self.getBody() : self.getDoc();

			switch (nativeName) {
				case 'contextmenu':
					dom.bind(user, nativeName, eventHandler);
					break;
	...
	tinymce.EditorCommands = function(editor) {
		...
		addCommands({
			...
			mceInsertContent : function(command, ui, value) {
				var parser, serializer, parentNode, userNode, fragment, args,
				...
				if (!args.invalid) {
					...
					parentNode = selection.getNode();
					userNode = editor.getBody();

					// Opera will return the document node when selection is in user
					if (parentNode.nodeType == 9)
						parentNode = node = userNode;
					else
						node = parentNode;

					// Find the ancestor just before the user element
					while (node !== userNode) {
						parentNode = node;
						node = node.parentNode;
					}

					// Get the outer/inner HTML depending on if we are in the user and parser and serialize that
					value = parentNode == userNode ? userNode.innerHTML : dom.getOuterHTML(parentNode);
					...
					// Set the inner/outer HTML depending on if we are in the user or not
					if (parentNode == userNode)
						dom.setHTML(userNode, value);
					else
			...
			'Indent,Outdent' : function(command) {
				...
				if (!queryCommandState('InsertUnorderedList') && !queryCommandState('InsertOrderedList')) {
					// If forced_user_blocks is set to false we don't have a block to indent so lets create a div
					if (!settings.forced_user_block && !dom.getParent(selection.getNode(), dom.isBlock)) {
						formatter.apply('div');
					}
			...
			selectAll : function() {
				var user = dom.getuser(), rng = dom.createRng();

				// Old IE does a better job with selectall than new versions
				if (selection.getRng().setStart) {
					rng.setStart(user, 0);
					rng.setEnd(user, user.childNodes.length);
...
(function(tinymce) {
	...
	tinymce.UndoManager = function(editor) {
		...
		editor.dom.bind(editor.dom.getuser(), 'dragend', addNonTypingUndoLevel);
...
tinymce.ForceBlocks = function(editor) {
	...
	function adduserBlocks() {
		var node = selection.getStart(), userNode = editor.getBody(), rng, startContainer, startOffset, endContainer, endOffset, userBlockNode, tempNode, offset = -0xFFFFFF, wrapped, isInEditorDocument;

		if (!node || node.nodeType !== 1 || !settings.forced_user_block)
			return;

		// Check if node is wrapped in block
		while (node && node != userNode) {
		...
		while (node) {
			if (node.nodeType === 3 || (node.nodeType == 1 && !blockElements[node.nodeName])) {
				...
				if (!userBlockNode) {
					userBlockNode = dom.create(settings.forced_user_block);
					node.parentNode.insertBefore(userBlockNode, node);
					wrapped = true;
				}

				tempNode = node;
				node = node.nextSibling;
				userBlockNode.appendChild(tempNode);
			} else {
				userBlockNode = null;
				node = node.nextSibling;
			}
		...
		if (wrapped) {
			if (rng.setStart) {
				...
				if (isInEditorDocument) {
					try {
						rng = editor.getDoc().body.createTextRange();
						rng.moveToElementText(userNode);
						rng.collapse(true);
	...
	// Force user blocks
	if (settings.forced_user_block) {
		editor.onKeyUp.add(adduserBlocks);
		editor.onNodeChange.add(adduserBlocks);
	}
};
...
(function(tinymce) {
	tinymce.Formatter = function(ed) {
		var formats = {},
			...
			isBlock = dom.isBlock,
			forceduserBlock = ed.settings.forced_user_block,
			nodeIndex = dom.nodeIndex,
			INVISIBLE_CHAR = '\uFEFF',
		...
		function getParents(node, selector) {
			return dom.getParents(node, selector, dom.getuser());
		};
			...
			if (format) {
				if (node) {
					...
					if (!isCollapsed || !format.inline || dom.select('td.mceSelected,th.mceSelected').length) {
						...
						// If the formats have a default block and we can't find a parent block then start wrapping it with a DIV this is for forced_user_blocks: false
						// It's kind of a hack but people should be using the default block type P since all desktop editors work that way
						if (!forceduserBlock && formatList[0].defaultBlock && !dom.getParent(curSelNode, dom.isBlock)) {
		...
		function remove(name, vars, node) {
			...
			function findFormatuser(container) {
				var formatuser;
				...
				each(getParents(container.parentNode).reverse(), function(parent) {
					...
					// Find format user element
					if (!formatuser && parent.id != '_start' && parent.id != '_end') {
			...
			function wrapAndSplit(format_user, container, target, split) {
				var parent, clone, lastClone, firstClone, i, formatuserParent;

				// Format user found then clone formats and split it
				if (format_user) {
					formatuserParent = format_user.parentNode;

					for (parent = container.parentNode; parent && parent != formatuserParent; parent = parent.parentNode) {
					...
					if (split && (!format.mixed || !isBlock(format_user)))
						container = dom.split(format_user, container);
			...
			function splitToFormatuser(container) {
				return wrapAndSplit(findFormatuser(container), container, container, true);
			};
			...
			function removeRngStyle(rng) {
				...
				if (format.split) {
					...
					if (startContainer != endContainer) {
						...
						// Split start/end
						splitToFormatuser(startContainer);
						splitToFormatuser(endContainer);

						// Unwrap start/end to get real elements again
						startContainer = unwrap(TRUE);
						endContainer = unwrap();
					} else
						startContainer = endContainer = splitToFormatuser(startContainer);
		...
		function expandRng(rng, format, remove) {
			...
			function findParentContainer(start) {
				var container, parent, child, sibling, siblingName, user;
				...
				for (;;) {
					...
					// Check if we can move up are we at user level or body level
					if (parent.parentNode == user) {
						container = parent;
						break;
					}
		...
		function removeNode(node, format) {
			var parentNode = node.parentNode, userBlockElm;
			...
			if (format.block) {
				if (!forceduserBlock) {
					...
					// Wrap the block in a forceduserBlock if we are at the user of document
					if (parentNode == dom.getuser()) {
						if (!format.list_block || !isEq(node, format.list_block)) {
							each(tinymce.grep(node.childNodes), function(node) {
								if (isValid(forceduserBlock, node.nodeName.toLowerCase())) {
									if (!userBlockElm)
										userBlockElm = wrap(node, forceduserBlock);
									else
										userBlockElm.appendChild(node);
								} else
									userBlockElm = 0;
							});
						}
...
(function(tinymce) {
	...
	tinymce.EnterKey = function(editor) {
		...
		function handleEnterKey(evt) {
			var rng = selection.getRng(true), tmpRng, editableuser, container, offset, parentBlock, documentMode, shiftKey,
			...
			// Moves the caret to a suitable position within the user for example in the first non pure whitespace text node or before an image
			function moveToCaretPosition(user) {
				var walker, node, rng, y, viewPort, lastNode = user, tempElm;

				rng = dom.createRng();

				if (user.hasChildNodes()) {
					walker = new TreeWalker(user, user);
					...
					if (user.nodeName == 'BR') {
						if (user.nextSibling && dom.isBlock(user.nextSibling)) {
							// Trick on older IE versions to render the caret before the BR between two lists
							if (!documentMode || documentMode < 9) {
								tempElm = dom.create('br');
								user.parentNode.insertBefore(tempElm, user);
							}

							rng.setStartBefore(user);
							rng.setEndBefore(user);
						} else {
							rng.setStartAfter(user);
							rng.setEndAfter(user);
						}
					} else {
						rng.setStart(user, 0);
						rng.setEnd(user, 0);
			...
			// Wraps any text nodes or inline elements in the specified forced user block name
			function wrapSelfAndSiblingsInDefaultBlock(container, offset) {
				...
				if (!parentBlock || !canSplitBlock(parentBlock)) {
					parentBlock = parentBlock || editableuser;
			...
			// Inserts a BR element if the forced_user_block option is set to false or empty string
			function insertBr() {
				var brElm, extraBr;
			...
			function getEditableuser(node) {
				var user = dom.getuser(), parent, editableuser;

				// Get all parents until we hit a non editable parent or the user
				parent = node;
				while (parent !== user && dom.getContentEditable(parent) !== "false") {
				...
				return parent !== user ? editableuser : user;
			};
			...
			container = rng.startContainer;
			offset = rng.startOffset;
			newBlockName = (settings.force_p_newlines ? 'p' : '') || settings.forced_user_block;
			...
			// Get editable user node normaly the body element but sometimes a div or span
			editableuser = getEditableuser(container);

			// If there is no editable user then enter is done inside a contentEditable false element
			if (!editableuser) {
				return;
			}
			...
			// If editable user isn't block nor the user of the editor
			if (!dom.isBlock(editableuser) && editableuser != dom.getuser()) {
			...
			// This won't happen if user blocks are disabled or the shiftKey is pressed
			...
			if (parentBlockName == 'PRE' && settings.br_in_pre !== false) {
				...
				// If no user block is configured then insert a BR by default or if the shiftKey is pressed
