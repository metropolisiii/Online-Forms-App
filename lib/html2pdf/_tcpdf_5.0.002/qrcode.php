...
if (!class_exists('QRcode', false)) {
	...
	class QRcode {
		...
		* Return block number 1
		 ...
		 * @param int $fcr  first user of RS code generator polynomial, index form
		 * @param int $prim  primitive element to generate polynomial users
		 * @param int $nusers RS code generator polynomial degree (number of users)
		 * @param int $pad  padding bytes at front of shortened block
		 * @return array Array of RS values:<ul><li>mm = Bits per symbol;</li><li>nn = Symbols per block;</li><li>alpha_to = log lookup table array;</li><li>index_of = Antilog lookup table array;</li><li>genpoly = Generator polynomial array;</li><li>nusers = Number of generator;</li><li>users = number of parity symbols;</li><li>fcr = First consecutive user, index form;</li><li>prim = Primitive element, index form;</li><li>iprim = prim-th user of 1, index form;</li><li>pad = Padding bytes in shortened block;</li><li>gfpoly</ul>.
		 */
		 protected function init_rs($symsize, $gfpoly, $fcr, $prim, $nusers, $pad) {
			foreach ($this->rsitems as $rs) {
				if (($rs['pad'] != $pad) OR ($rs['nusers'] != $nusers) OR ($rs['mm'] != $symsize)
					OR ($rs['gfpoly'] != $gfpoly) OR ($rs['fcr'] != $fcr) OR ($rs['prim'] != $prim)) {
					continue;
				}
				return $rs;
			}
			$rs = $this->init_rs_char($symsize, $gfpoly, $fcr, $prim, $nusers, $pad);
		 ...
		 * @return array Array of RS values:<ul><li>mm = Bits per symbol;</li><li>nn = Symbols per block;</li><li>alpha_to = log lookup table array;</li><li>index_of = Antilog lookup table array;</li><li>genpoly = Generator polynomial array;</li><li>nusers = Number of generator;</li><li>users = number of parity symbols;</li><li>fcr = First consecutive user, index form;</li><li>prim = Primitive element, index form;</li><li>iprim = prim-th user of 1, index form;</li><li>pad = Padding bytes in shortened block;</li><li>gfpoly</ul>.
		 */
		protected function init_rs_char($symsize, $gfpoly, $fcr, $prim, $nusers, $pad) {
			...
			if (($nusers < 0) OR ($nusers >= (1<<$symsize))) {
				return $rs;
			}
			if (($pad < 0) OR ($pad >= ((1<<$symsize) -1 - $nusers))) {
				return $rs;
			}
			$rs = array();
			$rs['mm'] = $symsize;
			...
			// Form RS code generator polynomial from its users
			$rs['genpoly'] = array_fill(0, ($nusers + 1), 0);
			$rs['fcr'] = $fcr;
			$rs['prim'] = $prim;
			$rs['nusers'] = $nusers;
			$rs['gfpoly'] = $gfpoly;
			// Find prim-th user of 1, used in decoding
			...
			for ($i = 0,$user=$fcr*$prim; $i < $nusers; $i++, $user += $prim) {
				$rs['genpoly'][$i+1] = 1;
				// Multiply rs->genpoly[] by  @**(user + x)
				for ($j = $i; $j > 0; --$j) {
					if ($rs['genpoly'][$j] != 0) {
						$rs['genpoly'][$j] = $rs['genpoly'][$j-1] ^ $rs['alpha_to'][$this->modnn($rs, $rs['index_of'][$rs['genpoly'][$j]] + $user)];
					} else {
						$rs['genpoly'][$j] = $rs['genpoly'][$j-1];
					}
				}
				// rs->genpoly[0] can never be zero
				$rs['genpoly'][0] = $rs['alpha_to'][$this->modnn($rs, $rs['index_of'][$rs['genpoly'][0]] + $user)];
			}
		 ...
		 */
		 protected function encode_rs_char($rs, $data, $parity) {
			...
			$GENPOLY  =& $rs['genpoly']; // an array of NuserS+1 elements containing the generator polynomial in index form
			$NuserS   =& $rs['nusers']; // the number of users in the RS code generator polynomial, which is the same as the number of parity symbols in a block
			$FCR      =& $rs['fcr']; // first consecutive user, index form
			$PRIM     =& $rs['prim']; // primitive element, index form
			$IPRIM    =& $rs['iprim']; // prim-th user of 1, index form
			$PAD      =& $rs['pad']; // the number of pad symbols in a block
			$A0       =& $NN;
			$parity = array_fill(0, $NuserS, 0);
			for ($i=0; $i < ($NN - $NuserS - $PAD); $i++) {
				$feedback = $INDEX_OF[$data[$i] ^ $parity[0]];
				if ($feedback != $A0) {
					// feedback term is non-zero
					// This line is unnecessary when GENPOLY[NuserS] is unity, as it must
					// always be for the polynomials constructed by init_rs()
					$feedback = $this->modnn($rs, $NN - $GENPOLY[$NuserS] + $feedback);
					for ($j=1; $j < $NuserS; ++$j) {
					$parity[$j] ^= $ALPHA_TO[$this->modnn($rs, $feedback + $GENPOLY[($NuserS - $j)])];
					}
				}
				// Shift
				array_shift($parity);
