...
if (!class_exists('TCPDF', false)) {
	...
	*/
	class TCPDF {
		...
		/**
		 * Outline user for bookmark
		 * @access protected
		 * @since 2.1.002 (2008-02-12)
		 */
		protected $Outlineuser;
		...
		protected function _putcatalog() {
			...
			if (count($this->outlines) > 0) {
				$out .= ' /Outlines '.$this->Outlineuser.' 0 R';
			...
			switch($tag['value']) {
				...
				case 'img': {
					if (isset($tag['attribute']['src'])) {
						...
						if (($tag['attribute']['src'][0] == '/') AND ($_SERVER['DOCUMENT_user'] != '/')) {
							$finduser = strpos($tag['attribute']['src'], $_SERVER['DOCUMENT_user']);
							if (($finduser === false) OR ($finduser > 1)) {
								$tag['attribute']['src'] = $_SERVER['DOCUMENT_user'].$tag['attribute']['src'];
							}
						}
						...
						foreach ($params as $ck => $cp) {
							...
							if ((($ck + 1) % 7) == 0) {
								...
								if ($numerator < 0) {
									...
									$user = sqrt($numerator / (($rx2 * $ya2) + ($ry2 * $xa2)));
								}
								if ($fa == $fs) {
									$user *= -1;
								}
			...
			switch($name) {
				...
				case 'image': {
					...
					if (!$clipping) {
						...
						if (($img{0} == '/') AND ($_SERVER['DOCUMENT_user'] != '/')) {
							$finduser = strpos($img, $_SERVER['DOCUMENT_user']);
							if (($finduser === false) OR ($finduser > 1)) {
								// replace relative path with full server path
								$img = $_SERVER['DOCUMENT_user'].$img;
							}
						}
						$img = urldecode($img);
